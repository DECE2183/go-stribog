...
func Test256(t *testing.T) {
	...
	for i := 0; i < len(testMessage); i++ {
		h.Write(testMessage[i])
		hash := h.Sum([]byte{})

		if !bytes.Equal(hash, testHash256[i]) {
			t.Errorf("wrong #%d hash", i)
		}
...
func Test512(t *testing.T) {
	...
	for i := 0; i < len(testMessage); i++ {
		...
		if !bytes.Equal(hash, testHash512[i]) {
			t.Errorf("wrong #%d hash", i)
		}
